(function() {
  ...
  define(["jquery", "underscore", "backbone", "rivets", 'kinetic', 'logger', "text!templates/pattern/show.html"], function($, _, Backbone, rivets, Kinetic, aLogger, PatternShowView) {
    var PatternShowViewTemplate;
    PatternShowViewTemplate = Backbone.View.extend({
      el: $("#right_board"),
      render: function() {
        var Box, Boxes, CollisionPair, CollisionUtil, Logger, StackBoard, compiledTemplate, data;
        Logger = aLogger.create;
        data = {
          title: "Pattern Designer"
        };
        compiledTemplate = _.template(PatternShowView, data);
        ...
        Box = (function(_super) {
          ...
          Box.prototype.rotateArrow = function(angle) {
            var vectorDegree;
            vectorDegree = angle;
            if (vectorDegree <= 360) {
              this.set('vectorEnabled', true);
              this.get('dot').setFillAlpha(0);
              this.get('arrow').strokeAlpha(1);
              this.set('vectorDegree', vectorDegree);
              return this.get('arrow').rotation(this.get('vectorDegree'));
            } else {
              this.set('vectorEnabled', false);
              this.get('dot').setFillAlpha(1);
              this.get('arrow').strokeAlpha(0);
              return this.set('vectorDegree', -45);
            }
          };
        ...
        Boxes = (function(_super) {
          ...
          Boxes.prototype.createNewBox = function(to_be_added_box_data) {
            var newBox;
            ...
            return Logger.debug("create new box completed!");
          };
          ...
          Boxes.prototype.settleCurrentBox = function() {
            ...
            if (this.currentBox.get('collisionStatus')) {
              ...
              return window.appController.sendUsedLayersToSave();
            }
          };
          ...
          Boxes.prototype.rotateByVector = function() {
            ...
            this.currentBox.rotateArrow(vectorDegree);
        ...
        StackBoard = (function() {
          ...
          StackBoard.prototype.saveLayerData = function(layer) {
            if (layer == null) {
              layer = {};
            }
            ...
            if (layer.boxes === void 0) {
              layer_data.boxes = [];
            } else {
              layer_data.boxes = _.map(this.boxes.models, (function(a_box) {
                return {
                  x: a_box.getXPosition(),
                  y: a_box.getYPosition(),
                  rotate: a_box.get('rotate'),
    ...
    return PatternShowViewTemplate;
  });
